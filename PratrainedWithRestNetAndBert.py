...
def PreProcessWithBert(data):
    ...
    for i in range(batch):
        if (i + 1) * 1000 < len(data):
            print("i:", i)
            token = tokenizer(data[i * 1000:(i + 1) * 1000], return_tensors='tf', max_length=100, padding='max_length',
            ...
            token = tokenizer(data[i * 1000:], return_tensors='tf', max_length=100, padding='max_length',
                              truncation=True, add_special_tokens=True)
            output = bert_model(token)
            with tf.compat.v1.Session() as sess:
                sess.run(tf.compat.v1.global_variables_initializer())
...
def createModel(train_data):
    ...
    lay = AveragePooling1D(pool_size=5,strides=None,padding='valid')(inputLayer)
...
def TrainModel(model, train_data, train_label, test_data, test_label):
    earlyStop = EarlyStopping(monitor='val_accuracy', min_delta=0, patience=50, mode='max', verbose=1,
...
def TrainBiLSTMModel(model, train_data, train_label, test_data, test_label):
    earlyStop = EarlyStopping(monitor='val_accuracy', min_delta=0, patience=100, mode='max', verbose=1,
                              restore_best_weights=True)
    histoty = model.fit(
        x=train_data,
        y=train_label,
            ...
            if content[1] == content[2] or content[1] == content[3] or content[2] == content[3]:  # 三对标签至少有两对才能去
                ...
                if canRead == True:
                    with open(dataPath + '/' + content[0] + '.txt', 'r', encoding='utf-8') as t:
                        line = t.readline()
                        re_tag = re.compile(
                            r'(#\S+)|(\s{2,})|(@)|((http|ftp|https):\/\/[\w\-_]+(\.[\w\-_]+)+([\w\-\.,@?^=%&:/~\+#]*[\w\-\@?^=%&/~\+#])?)')
                        re_tag.sub('', line)
...
def RestNetFineTune(image_path, label):
    ...
    sgd = optimizers.gradient_descent_v2.SGD(learning_rate=0.001)
    model.compile(loss='categorical_crossentropy', optimizer=sgd, metrics=['accuracy'])
    earlyStop = EarlyStopping(monitor='val_accuracy', min_delta=0, patience=5, mode='max', verbose=10,
                              restore_best_weights=True)
    history = model.fit(x=train_data, y=train_label, validation_split=0.2, batch_size=120, epochs=50, verbose=1,
                        callbacks=[earlyStop])
